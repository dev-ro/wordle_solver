...
def calculate_letter_frequency(word_list: List[str], length: int, prefix: Optional[str] = None) -> Dict[str, float]:
    """
    ...
    return dict(sorted(percentage_frequency.items(), key=lambda item: item[1], reverse=True))
...
def calculate_guess_score(word: str, letter_scores: Dict[str, float], guess_count: int = 1) -> float:
    """
...
def filter_possible_words(word_list: List[str], guess_feedback: List[Tuple[str, str]]) -> List[str]:
    """
    ...
    def is_word_possible(word: str) -> bool:
        ...
        word_letter_counts = {letter: word.count(letter) for letter, _ in guess_feedback}
        
        for i, (letter, feedback) in enumerate(guess_feedback):
            if feedback == "g":  # Green: letter must match at this position
                if (word[i] != letter or 
                    word_letter_counts[letter] < correct_letter_counts[letter]):
                    return False
            elif feedback == "y":  # Yellow: letter in word, not at this position
                if (letter not in word or 
                    word[i] == letter or 
                    word_letter_counts[letter] <= correct_letter_counts[letter]):
                    return False
            elif feedback == "b":  # Black: letter should not be in word (considering g/y counts)
                if (letter in word and 
                    word_letter_counts[letter] > (correct_letter_counts[letter] + present_letter_counts[letter])):
                    return False
        return True
...
def recommend_guesses(word_list: List[str], length: int, prefix: Optional[str], n: int = 9, guess_count: int = 1) -> List[Tuple[str, float]]:
    """
...
def find_variable_letter_positions(word_list: List[str]) -> Dict[int, set]:
    ...
    return {
        pos: letters for pos, letters in variable_positions.items() 
        if len(letters) > 1
    }
...
def calculate_next_move(req: https_fn.CallableRequest) -> Dict[str, Any]:
    ...
    try:
        ...
        if variable_letters and len(possible_words) > 10:
            ...
            filler_candidates = [
                word for word in dictionary
                if len(word) == word_length and any(letter in word for letter in variable_letters)
            ]
