...
def _build_counts(
    guess_feedback: List[Tuple[str, str]],
...
def _validate_word_against_one_guess(
    candidate: str, guess_feedback: List[Tuple[str, str]]
) -> Tuple[bool, str | None]:
    ...
    for idx, (letter, fb) in enumerate(guess_feedback):
        if fb == "g":
            ...
            if letter in candidate and word_letter_counts.get(letter, 0) > (
                correct_counts.get(letter, 0) + present_counts.get(letter, 0)
            ):
                return False, f"exceeds allowed count of letter {letter}"
    return True, None


def is_word_possible(
    candidate: str, history: List[Dict[str, str]]
) -> Tuple[bool, List[str]]:
    reasons: List[str] = []
    for entry in history:
        ...
        if (
            not guess
            or not fb
            or len(guess) != len(candidate)
            or len(fb) != len(candidate)
        ):
...
def _write_dictionary(bucket_name: str, dictionary_path: str, words: List[str]) -> None:
    ...
    blob.upload_from_string(
        json.dumps(words, ensure_ascii=False), content_type="application/json"
    )
    blob.cache_control = "public, max-age=3600"
    blob.patch()


def _pending_submissions(db: firestore.Client) -> List[Tuple[Any, Dict]]:
...
def validate_and_publish(req: https_fn.Request) -> https_fn.Response:
    ...
    for doc_ref, sub in _pending_submissions(db):
        try:
    ...
    return https_fn.Response(
        json.dumps({"processed": processed}), mimetype="application/json"
    )
